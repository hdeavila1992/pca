...
#pos_cond: Indica donde se encuentra los puntos de control libres, es decir los que el algoritmo canbia 
#x: Poblacion de puntos de control a cambiar 
#Pinip: Son los puntos de control Isogeometricos 
	Long=np.size(pos_cond,1)
	#print(Long)	
	#print(pos_cond)
	#print(x)
	for i in range(Long): 
		#Des comentar si se requiere analisis de puntos en x
		#Pinip[pos_cond[0][i],0]=x[j,i]#x[j,(i+1)]  print(x[j,(0+Long)])
		#Pinip[pos_cond[0][i],1]=x[j,i+Long]
		Pinip[pos_cond[0][i],1]=x[j,i]
		#Descomentar si se requiere analisis de puntos en y
...
#Esclisov para el primer problema del articulo

def FPenal():
	#Continuar mañana Ojo	
...
def funcion(x):
	...
	Area_max=np.zeros(np.size(x,0))
	Uy_max=np.zeros(np.size(x,0))
	Ux_max=np.zeros(np.size(x,0))
	sy_max=np.zeros(np.size(x,0))
	sx_max=np.zeros(np.size(x,0))
	sxy_max=np.zeros(np.size(x,0))
	for j in range(np.size(x,0)):
		print('Puntos de control iniciales')
		print (Pinp)
		temp=Pinp
		...
		Vect_temp_sy=np.loadtxt('sy_data.dat')
		Vect_temp_sx=np.loadtxt('sx_data.dat')
		Vect_temp_sxy=np.loadtxt('sxy_data.dat')
		Vect_temp_uy=np.loadtxt('Uy_data.dat')
		Vect_temp_ux=np.loadtxt('Ux_data.dat')
		Vect_temp_svm=np.loadtxt('svm_data.dat')
		Area_max[j]=Vect_temp[1]
		Von_max[j]=Vect_temp_svm[1]
		Uy_max[j]=Vect_temp_uy[1]
		Ux_max[j]=Vect_temp_ux[1]
		sy_max[j]=Vect_temp_sy[1]
		sx_max[j]=Vect_temp_sx[1]
		sxy_max[j]=Vect_temp_sxy[1]
...
exec(open("Example1_cantilever_beam.py").read())
exec(open("RunIt.py").read())
...
New_ps=np.size(Pinp_x)


Base=np.zeros((New_ps,2))
Base[:,0]=1.0
...
x=np.zeros( (ps,np.size(Pinp_x,1)) )
print(x)

for i in range(ps):
	x[i,:]=Pinp_x+(random.rand(1,New_ps)-0.5)*2.0*rango
...
#while any(TerC)==False:
	for j in range(1):
		...
		(x,F)=Teaching(x,funcion,ps,1,FPenal)
		c1=(g1(x)<=0)*1
		c2=(g2(x)<=0)*1
